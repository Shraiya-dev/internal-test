...
export const WorkerProfileProvider = ({ children }) => {
    ...
    const { isLoading, isSuccess, data, error } = useGetWorkerProfile(workerId, bookingId)

    const [editWorkerState, dispatchEditWorkerState] = useReducer(editWorkerReducer, null)

    const [workerProfileError, setWorkerProfileError] = useState(null)

    const onNameChange = (e) => dispatchEditWorkerState({ type: 'SET_NAME', payload: e.target.value })
    ...
    const onTabClicked = (tabId) => {
        if (editMode || editVaccinationCert) {
            return toast.warning('Please Save or Cancel before moving to next tab', toastConfig)
        }
        setActiveTab(tabId)
    }
    ...
    const onSaveClicked = () => {
        ...
        const isValid = validateWorkerProfile(editWorkerState, setWorkerProfileError)
        if (!isValid) {
            return
        }
